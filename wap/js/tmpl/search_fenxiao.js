$(function() {
	...
	$("#keyword").on("input", function() {
		...
		if (e == "") {
			...
			$.getJSON(ApiUrl + "/index.php?act=goods&op=auto_complete", {
				term: $("#keyword").val()
			}, function(e) {
				if (!e.datas.error) {
					var t = e.datas;
	...
	$("#header-nav").click(function() {
		if ($("#keyword").val() == "") {
			window.location.href = buildUrl("keyword_fenxiao", getCookie("deft_key_value") ? getCookie("deft_key_value") : "")
		} else {
			window.location.href = buildUrl("keyword_fenxiao", $("#keyword").val())
		}
	})
});